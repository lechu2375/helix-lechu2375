...
if CLIENT then
	...
	function PANEL:AddText( str )
		...
		lab:SetText( str or "Missing text" )
		lab:SetWrap( true )
...
function PANEL:SetEntity( entity )
		...
		playerd:SetFont("ixMonoMediumFont")
	...
	---------------------
		...
		faction:SetFont("ixMonoMediumFont")
		...
		classes:SetFont("ixMonoMediumFont")
		...
		model:SetFont("ixMonoMediumFont")
		...
		self.save.DoClick = function()
			if (IsValid(entity) and (self.nextSend or 0) < CurTime()) then
				...
				netstream.Start("ix_DialogueData", { entity, dialogue, factionData, classData, self.name:GetText(), self.desc:GetText(), self.model:GetText() or entity:GetModel() } )
				self:Close()
			end
		end
end
...
function PLUGIN:DrawTargetID(entity, x, y, alpha)
	if (string.lower(entity:GetClass()) == "ix_talker") then
		local mainColor = ix.config.mainColor
		local color = Color(mainColor.r, mainColor.g, mainColor.b, alpha)

		ix.util.DrawText(x, y, entity:GetNetVar("name", "John Doe"), color)
			y = y + ix.config.targetTall
		ix.util.DrawText(x, y, entity:GetNetVar("desc", ix.lang.Get("no_desc")), Color(255, 255, 255, alpha), "ix_TargetFontSmall")
	end
end